...
func moon_mean_elongation_from_sun(jd2000: Double) -> Revolutions {
  ...
  _mean_elongation_from_sun = _mean_elongation_from_sun - Int(
    _mean_elongation_from_sun).double
...
func longitude_lunar_ascending_node(jd2000: Double) -> Revolutions {
  let _longitude_lunar_ascending_node = moon_mean_longitude(
    jd2000: jd2000) - moon_argument_of_latitude(jd2000: jd2000)
  ...
  observer: Observer) -> (rise: DateComponents?, set: DateComponents?)
{
  ...
  for hour in 0..<24 {
    ...
    if case .noTransit(let noTransit) = transit_info {
      moon_position_window[2].distance = noTransit.parallax
    } else if case .transitEvent(let transit) = transit_info {
      ...
      if transit.event == "rise" {
        ...
        if rise_time == nil {
          ...
          update_rise_time = update_rise_time || (rq_diff.sign != eq_diff.sign) &&
            (set_time != nil && rq_diff.sign == sq_diff.sign)
