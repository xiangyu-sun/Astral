...
class SolarTermDaysTests: XCTestCase {
  ...
  func testDaysUntilNextSolarTermMonotonicity() {
    ...
    XCTAssertLessThan(
      daysNow,
      adjustedDaysLater,
      "Days left should increase in the continuous cycle as time advances (after unwrapping).")
  }
  ...
  func testAll24SolarTermsAppear() {
    ...
    XCTAssertEqual(
      uniqueTerms.count,
      24,
      "Expected to see all 24 unique solar terms over a full cycle, but got \(uniqueTerms.count): \(uniqueTerms)")
  }
